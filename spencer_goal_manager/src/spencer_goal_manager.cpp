...
void guideGroup(string gate, bool send_result) {
	...
	ROS_INFO("SPENCER_GOAL_MANAGER request sent waiting for result");
	while (!has_finished && !isStopped()) {
		has_finished=guide_group_client_->waitForResult(ros::Duration(0.3));
	}
	if (!has_finished) {
		ROS_INFO("SPENCER_GOAL_MANAGER robot stopped");
		guide_group_client_->cancelGoal();
		guide_group_client_->waitForResult();
		setStopped(false); 
	}
	...
	ROS_INFO("SPENCER_GOAL_MANAGER request completed");
	if (send_result) {
...
bool moveToDestination(string dest,bool send_result) {
	...
	// status_pub.publish(status_msg);
	ROS_INFO("SPENCER_GOAL_MANAGER received request to move robot");
	move_to_goal.destination=dest;
	...
	if (!has_finished) { 
		ROS_INFO("SPENCER_GOAL_MANAGER stopped request");
	...
	ROS_INFO("SPENCER_GOAL_MANAGER Request completed");
	if (send_result) {
		ROS_INFO("Sending result %s",move_to_client_->getResult()->status.c_str());
		sendResult(move_to_client_->getResult()->status,client_command_socket_);
		
	}
	return move_to_client_->getResult()->status=="COMPLETED"; 
...
bool informAndGuide(string old_gate, string new_gate) {
	...
	if (has_moved) {
		...
		sendResult("FAILED",client_command_socket_);
	}
}
...
void commandLoop() {
	    ...
	    if (bind(server_socket, (struct sockaddr *) &serv_addr, 
	     ...
	     while (ros::ok()) {
		     ...
		     if (parameters[0]=="e") {
		     	ROS_INFO("SPENCER_GOAL_MANAGER Received request to exit");
	     		setShouldQuit(true);
		     	break;
		     }
		     if (parameters[0]=="m") {
   ...
   if (bind(server_socket, (struct sockaddr *) &serv_addr, 
    ...
    while (!shouldQuit()) {
		...
		if (s=="s") {
			...
			if (parameters.size()==2) {
				t=boost::thread(&changeGate,parameters[1]);
			}
			else {
				ROS_ERROR("SPENCER_GOAL_MANAGER WRONG_PARAMETERS");
...
void statusLoop() {
	...
	while(ros::ok() && !shouldQuit()) {
		string status=status_manager_->getStatus();
		string location=status_manager_->getLocation();
		// ROS_INFO("Status is %s",status.c_str());
		// ROS_INFO("Location is %s",location.c_str());

		sendResult(status,client_status_socket_);
		n=read(client_status_socket_,buffer,255);
		
		sendResult(location,client_status_socket_);
		n=read(client_status_socket_,buffer,255);
...
int main(int argc, char** argv) {
	...
	ROS_INFO("SPENCER_GOAL_MANAGER waiting for guide group and move to action servers to be up");
	...
	ROS_INFO("SPENCER_GOAL_MANAGER ready");
